var blog_express_upload_frame_obj;
...
( function( $ ) {
	...
	$(document).ready(function($) {
		...
		$(document).on('click', 'input.wh-image-picker', function( event ){
			...
			var blogExpressImageObject = wp.media.controller.Library.extend({
				defaults : _.defaults({
					id: 'blog-express-state-insert',
			...
			blog_express_upload_frame_obj = wp.media.frames.blog_express_upload_frame_obj = wp.media({
				button: {
					text: $( this ).data( 'uploader_button_text' )
				},
				state : 'blog-express-state-insert',
				states : [
					new blogExpressImageObject()
				],
				multiple: false
			});

			blog_express_upload_frame_obj.on( 'select', function() {

				var state = blog_express_upload_frame_obj.state('blog-express-state-insert');
			...
			blog_express_upload_frame_obj.open();
		});
